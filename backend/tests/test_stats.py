...
class TestAdminStats:
    ...
    async def test_admin_stats_no_longer_includes_public_fields(self, client, fake_redis):
        token = "token-no-public-fields"
        await fake_redis.set(f"admin_token:{token}", "1", ex=86400)
        response = await client.get(
            "/api/v1/stats/admin", headers={"X-Admin-Token": token}
        )
        data = response.json()
        assert "reaction_breakdown" not in data
        assert "cards_per_column" not in data
        assert "engagement_funnel" not in data
        assert "session_lifetime" not in data
        assert "activity_heatmap" not in data
